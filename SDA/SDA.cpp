...
int SDA::create() {
    ...
    initChar = std::uniform_int_distribution<int>(0, numChars - 1)(rng);
    ...
    for (int state = 0; state < numStates; ++state) {
        ...
        for (int val = 0; val < numChars; ++val) {
            oneState.push_back(static_cast<int>(std::uniform_int_distribution<int>(0, numStates - 1)(rng)));
        }
    ...
    for (int state = 0; state < numStates; ++state) {
        ...
        for (int trans = 0; trans < numChars; ++trans) {
            oneResponse.clear();
            const int respSize = std::uniform_int_distribution<int>(1, maxRespLen)(rng);;
            for (int val = 0; val < respSize; ++val) {
                oneResponse.push_back(std::uniform_int_distribution<int>(0, numChars - 1)(rng));
            }
...
/**
 * Takes a genome (vector of doubles [0.0, 1.0]) and forces it into the SDA.
 * Uses THRESHOLDS instead of Modulo for better evolution.
 */

int SDA::setGenes(const vector<double> &genes) {
    int geneIdx = 0;

    // 1. Initial Character
    if (geneIdx < genes.size()) {
        initChar = (int)(genes[geneIdx] * numChars);
        // cout  << initChar;
        if (initChar >= numChars) initChar = numChars - 1;
    ...
    for (int state = 0; state < numStates; ++state) {
        for (int trans = 0; trans < numChars; ++trans) {
            if (geneIdx < genes.size()) {
                int val = (int)(genes[geneIdx] * numStates);
                if (val >= numStates) val = numStates - 1;
                transitions[state][trans] = val;
                geneIdx++;
            }
        }
    }
...
int SDA::randomize() {
    ...
    for (int state = 0; state < numStates; ++state) {
        for (int trans = 0; trans < numChars; ++trans) {
            ...
            transitions[state][trans] = std::uniform_int_distribution<int>(0, numStates - 1)(rng);

            // Clear and generate a new response string for this transition.
            oneResponse.clear();
            const int respLen = std::uniform_int_distribution<int>(1, maxRespLen)(rng);
...
int SDA::crossover(SDA &other) {
    ...
    do {
        crossStart = std::uniform_int_distribution<int>(0, numStates)(rng); // 0 <= crossStart <= numStates
        crossEnd = std::uniform_int_distribution<int>(0, numStates)(rng); // 0 <= crossEnd <= numStates
...
int SDA::mutate(const int numMutations) {
    ...
    for (int mut = 0; mut < numMutations; ++mut) {
        ...
        if (std::bernoulli_distribution(0.04)(rng)) {
            initChar = std::uniform_int_distribution<int>(0, numChars - 1)(rng);
            if (verbose) {
        ...
        const int mutPt = std::uniform_int_distribution<int>(0, numStates - 1)(rng);
        const int transNum = std::uniform_int_distribution<int>(0, numChars - 1)(rng);

        // Mutate transition (48%)
        if (std::bernoulli_distribution(0.5)(rng)) {
            transitions.at(mutPt).at(transNum) = std::uniform_int_distribution<int>(0, numStates - 1)(rng);
            if (verbose) {
